...
const ELEMENT_IDS = {
    ...
    CALIB_MODE_SLIDER: 'calibModeSlider',
    CALIB_MODE_INDICATOR: 'calibModeIndicator',
    MARKER_SIZE: 'markerSize',
    START_ARUCO_CALIB: 'startArucoCalib',
    ARUCO_PANEL: 'arucoPanel',
...
class CamRuler {
    constructor() {
        ...
        this.calibrationMode = 'manual'; // 'manual', 'aruco', 'chessboard'
    ...
    setupCalibrationControls() {
        ...
        if (clearChessBtn) {
            startChessBtn.addEventListener('click', () => this.clearChessboardImages());
        }
    }
    ...
    animate() {
        ...
        // Process ArUco calibration if active
        if (this.isCalibrating && this.calibrationMode === 'aruco') {
            this.processArucoCalibration();
        }
            ...
            for (let i = 0; i < contours.size(); i++) {
                ...
                if (isCircular) {
                    ...
                    try {
                        ...
                        const radiusConv = circleRadius * Math.abs(this.calD[this.dm]);
